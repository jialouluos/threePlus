...
type RenderSetting<T extends object> = {
	[key in keyof T]?: T[key] extends object ? RenderSetting<T[key]> : T[key];
};
...
export default class Main extends EventEmitter<I_Event> {
	...
	/**渲染器参数 */
	rendererOption: {
		config: THREE.WebGLRendererParameters;
		setting: Partial<THREE.WebGLRenderer>;
	...
	createRenderer(params?: {
		config?: THREE.WebGLRendererParameters;
		setting?: RenderSetting<THREE.WebGLRenderer>;
		options?: RendererParams;
	}): void {
		const { config, setting, options } = this.rendererOption;
		...
		for (const [key, value] of Object.entries({
			...
			if (typeof this.renderer[key] === 'object') {
				for (const [key2, value2] of Object.entries(value)) this.renderer[key][key2] = value2;
			} else {
				this.renderer[key] = value;
			}
		}
		this.renderer.setClearColor(params?.options?.backgroundColor ?? options.backgroundColor);
	...
	createLight(count?: 1 | 2): void {
		...
		if (count) {
			if (count) {
				const dirLight = new THREE.DirectionalLight(0xff8888, 1);
				dirLight.position.set(200, 300, 400);
				this.lightGroups.add(dirLight);
				count--;
			}
			if (count) {
				const dirLight2 = new THREE.DirectionalLight('#fff', 1);
	...
	createControls(cb?: (controls: OrbitControls) => void): void {
		...
		if (this.camera instanceof THREE.PerspectiveCamera) this._controls.target.copy(this.perspectiveOption.target);
		else {
	...
	private _createPerspectiveCamera(params: PerspectiveOption): THREE.PerspectiveCamera {
		...
		params.autoFov &&
			(config.fov = Main.math.rad2Deg(2 * Math.atan(this.container.clientHeight / 2 / config.position.z)));
	...
	private _updatePerspectiveCamera(params: PerspectiveOption): void {
		if (this.camera instanceof THREE.PerspectiveCamera) {
			...
			params.autoFov &&
				(this.perspectiveOption.fov = Main.math.rad2Deg(
					2 * Math.atan(this.container.clientHeight / 2 / this.perspectiveOption.position.z)
				));
