...
export default class mathPlus {
	...
	getSymmetricalTime = (lowRange: number, highRange: number, timeScale: number = 1.0) => ({
		get value() {
			return (
				(0.5 - Math.abs(mathPlus._time * timeScale - Math.floor(mathPlus._time * timeScale) - 0.5)) *
					2.0 *
					(highRange - lowRange) +
				lowRange
			);
	...
	getGeometryByteLength = (geometry: THREE.BufferGeometry) => {
		...
		if (geometry.index) total += (geometry.index.array as unknown as ArrayBuffer).byteLength;

		for (const name in geometry.attributes) {
			total += (geometry.attributes[name].array as unknown as ArrayBuffer).byteLength;
		}
