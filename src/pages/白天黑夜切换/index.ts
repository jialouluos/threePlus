...
uniform float u_Time;`
  ...
  addLensflare() {
    ...
    this.lensflare.addElement(
      new LensflareElement(textureFlare0, 130, 0, light.color)
    )
  ...
  onSceneBeforeUpdate(): void {
    this.sun &&
      this.sun.position.set(
        Math.cos(this.time2.value * Math.PI) * 10000,
        Math.sin(this.time2.value * Math.PI) * 10000,
        0
      )
    ...
    this.renderer.toneMappingExposure =
      this.time.value > 0.2 ? this.time.value : 0.2
  }
}
